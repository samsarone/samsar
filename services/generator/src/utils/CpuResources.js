...
export function resolvePerTaskCpuUpperBound(
  configuredValue,
  defaultUpperLimit,
  {
  ...
  const concurrentTasks = Math.min(
    parsePositiveInteger(maxConcurrentTasks) ?? 1,
    available,
  );
  const perTaskCpuBudget = Math.max(1, Math.floor(available / concurrentTasks));

  return resolveCpuUpperBound(configuredValue, defaultUpperLimit, {
    availableCpuCount: perTaskCpuBudget,
  });
}
