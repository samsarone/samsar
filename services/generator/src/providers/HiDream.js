...
export async function handleHiDreamRequest(payload) {
  ...
  if (apiGenerationStatus === 'INIT') {
    await submitHiDreamRequest(payload);
  } else if (apiGenerationStatus === 'PENDING') {
    const imageData = await pollHiDreamRequest(payload);
...
export async function submitHiDreamRequest(payload) {
  ...
  const listNegativePrompts = "nsfw, nude, nipples, loli,shoti, watermark, copyright, logo, signature";
...
export async function pollHiDreamRequest(payload) {
