...
export async function handleLuminaRequest(payload) {

  const { apiGenerationStatus, apiRequestId, model } = payload;


  if (apiGenerationStatus === 'INIT') {
    await submitLuminaRequest(payload);
  } else if (apiGenerationStatus === 'PENDING') {
    const imageData = await pollLuminaRequest(payload);
...
export async function submitLuminaRequest(payload) {
...
export async function pollLuminaRequest(payload) {
