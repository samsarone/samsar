...
export async function handleFluxRequest(payload) {
  ...
  if (apiGenerationStatus === 'INIT') {
    await submitFluxRequest(payload);
...
export async function submitFluxRequest(payload) {
...
function getFalLinkForModel(model) {
  ...
  } else if (model === 'FLUX1PRO') {
    return "fal-ai/flux-pro/new";
  } else if (model === 'FLUX1.1PRO') {
    return "fal-ai/flux-pro/v1.1";
  } else if (model === 'FLUX1.1ULTRA') {
