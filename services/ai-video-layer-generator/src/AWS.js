...
function buildMediaDeliveryUrl(key) {
  if (isDockerRuntime() && !shouldUseDockerLocalMedia()) {
    assertConfiguredDockerExternalMediaDelivery(key);
  }
...
function getStaticCdnHostnames() {
  const hosts = new Set(
    isDockerRuntime()
      ? []
      : ['static.samsar.one', 'dgyheyjs5bch6.cloudfront.net'],
  );
  const configuredValues = isDockerRuntime()
    ? [process.env.STATIC_CDN_URL, process.env.PUBLIC_STATIC_CDN_URL, process.env.LEGACY_STATIC_CDN_URL]
    : [STATIC_CDN_URL, process.env.PUBLIC_STATIC_CDN_URL, process.env.LEGACY_STATIC_CDN_URL];
...
function getS3MediaKeyFromUrl(url) {
  const configuredBucketName = isDockerRuntime()
    ? normalizeString(process.env.MEDIA_BUCKET_NAME || process.env.STATIC_CDN_BUCKET)
    : bucketName;
  ...
  if (
    hostname === `${configuredBucketName}.s3.amazonaws.com` ||
    hostname.startsWith(`${configuredBucketName}.s3.`)
  ) {
    return pathname;
  }

  if (hostname === 's3.amazonaws.com' || hostname.startsWith('s3.')) {
    return pathname.startsWith(`${configuredBucketName}/`)
      ? pathname.slice(configuredBucketName.length + 1)
      : null;
  }
...
export async function normalizeProviderMediaUrl(value, options = {}) {
  ...
  if (isDockerRuntime()) {
    ...
    // A public URL can legitimately expose a root-level `assets_v2/...` path
    // without belonging to this Docker install. Only canonicalize an HTTP URL
    // after ownership has produced a media-key reference (or a mounted file was
    // actually resolved); otherwise preserve it as an independent public URL.
    ...
    // An independently public URL is already provider-readable. Do not pass it
    // through legacy signing/default-CDN logic after Docker ownership checks.
    return normalizedValue;
  }
...
function getS3Client() {
  if (isDockerRuntime() && !shouldUseDockerLocalMedia()) {
    assertConfiguredDockerExternalMediaDelivery('S3 media upload');
  }
