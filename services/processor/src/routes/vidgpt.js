...
router.post('/create', async function (req, res) {
  ...
  try {
    ...
    res.status(500).send({ error: "Failed to create VidGPT session" });
  }

});

router.post('/create_new_session_with_body', async function (req, res) {
  const headers = req.headers;
  const payload = req.body;
  const userId = verifyUserAuth(req.headers);
  if (!userId) {
  ...
  try {
    ...
    console.error("Error creating VidGPT session with body:", error);
    res.status(500).send({ error: "Failed to create VidGPT session with body" });
  }

});


router.post('/create_info_vid', async function (req, res) {
  const headers = req.headers;
  const payload = req.body;
  const userId = verifyUserAuth(req.headers);
  if (!userId) {
