...
test('Samsar fallback availability includes Qwen 3.8 and media models', () => {
  const available = buildAvailableDeploymentModels({
    samsar: { ok: true, status: 'valid' },
  });

  assert.equal(DEPLOYMENT_PROVIDER_CAPABILITIES.samsar.models.includes('QWEN3.8'), true);
  assert.equal(available.models.includes('QWEN3.8'), true);
