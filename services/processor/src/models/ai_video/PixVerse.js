...
export async function requestRenderPixVerseVideo(payload) {
  ...
  const { videoSessionId, currentLayerId, prompt, combineLayers, useStartFrame,
    useEndFrame, aspectRatio, model,
    
    modelSubType,
    clipLayerToAiVideo, userId, duration } = payload;
  ...
  aiVideoRenderPayload.animationType = payload.modelSubType;





  const aiVideoRenderPayloadDes = new AIVideoLayerGeneration(aiVideoRenderPayload);

  await aiVideoRenderPayloadDes.save();
