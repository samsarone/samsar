...
function normalizeRequestBodyLimit(value) {
...
export function getKimiK3SerializedRequestBodyBytes(payload) {
...
export function assertKimiK3RequestBodySize(
  ...
  const limit = normalizeRequestBodyLimit(maxRequestBodyBytes);
  const serializedBytes = getKimiK3SerializedRequestBodyBytes(payload);
  if (serializedBytes >= limit) {
    throw new Error('Kimi K3 request body must be smaller than the 100 MB request limit.');
  }
...
function getMimeTypeFromReference(reference, fallback) {
  const normalized = normalizeString(reference).split(/[?#]/, 1)[0].toLowerCase();
...
async function readRemoteMedia(reference, {
  ...
  if (buffer.length >= KIMI_K3_MAX_REQUEST_BODY_BYTES) {
...
async function normalizeKimiContentPart(part, context) {
  ...
  if (type === 'image' || type === 'input_image' || type === 'image_url') {
    ...
    context.inlineMediaBytes += Buffer.byteLength(inlineURL, 'utf8');
    if (context.inlineMediaBytes >= context.maxRequestBodyBytes) {
...
export async function buildKimiK3ChatRequest(
  ...
  const context = {
    ...
    uploadedFileIds: [],
    inlineMediaBytes: 0,
    maxRequestBodyBytes: normalizeRequestBodyLimit(
      dependencyOverrides.maxRequestBodyBytes,
    ),
  };
  try {
    const resolvedPayload = await resolveProviderMediaPayload(
      { messages: messages ?? input ?? [] },
      {
        resolveMediaUrl: dependencyOverrides.resolveMediaUrl,
        serviceName: 'samsar_processor_kimi_k3',
      },
    );
    ...
    assertKimiK3RequestBodySize(payload, context.maxRequestBodyBytes);
