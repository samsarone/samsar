...
export function getPresetAnimationListForDistribution(
  ...
  const endDistEnd = distribution[distribution.length - 1].endFrame + 30;
  ...
  const initTranslateX = canvasWidth * 0.1;
  const initTranslateY = canvasHeight * 0.1;


  // Initialize cumulative transformations
  let cumulativeScale = 120;
  let cumulativeSlideX = -initTranslateX;
  let cumulativeSlideY = -initTranslateY;
