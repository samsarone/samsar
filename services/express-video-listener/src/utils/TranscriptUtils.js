...
export async function addSubtitlesForSessionForAudio(sessionId, audioLayerId, rawLayers) {
  ...
  const targetAudioLayerId = audioLayerId?.toString?.() || String(audioLayerId);

  // Remove existing subtitles for this audio layer to avoid duplicates/stale timings
  for (const layer of sessionLayers) {
    ...
    const filteredItems = activeItemList.filter((item) => {
      if (!item) return false;
      if (item.type !== 'text' || item.subType !== 'subtitle') {
        return true;
      }
      const itemAudioLayerId = item?.audioLayerId?.toString?.() || String(item?.audioLayerId || '');
      return itemAudioLayerId !== targetAudioLayerId;
    });

    if (filteredItems.length !== activeItemList.length) {
      layer.imageSession.activeItemList = filteredItems;
