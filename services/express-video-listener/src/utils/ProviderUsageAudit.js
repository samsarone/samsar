...
import { isStandaloneEdition } from './EnvironmentUtils.js';
...
export function resolveDockerJobType(payload = {}) {
  if (payload.jobType) return normalizeString(payload.jobType);
  if (payload.isExpressGeneration || payload.expressGeneration || payload.expressJob) return 'Express video';
  if (payload.isMovieGen || payload.isVidGPTGen) return 'Image list to video';
...
export async function recordProviderUsageLog(input = {}) {
  ...
  const service = normalizeString(input.service || process.env.SERVICE_NAME || 'samsar_express_video_listener');
  const idempotencyKey = normalizeString(input.idempotencyKey) ||
    [service, localRequestId, requestType, provider, model].filter(Boolean).join(':');
