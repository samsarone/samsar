...
`OPENROUTER_QWEN_37_PLUS_MODEL` overrides that mapping. Native Alibaba routing uses `qwen3.7-max` for text-only inference and `qwen3.7-plus` when the request contains image or video input. For Docker installations configured with a Token Plan endpoint, native text inference instead uses `qwen3.8-max-preview`; the Samsar client labels that effective route as Qwen 3.8. OpenRouter requests explicitly use high reasoning for Qwen and Gemini and `xhigh` for GPT, with a 65,536-token completion allowance and a 10-minute minimum timeout. All external-assistant providers apply up to three exponential-backoff retries for transient, rate-limit, and malformed-response errors and honor provider `Retry-After` guidance. Payment-required and insufficient-credit failures are never retried.
